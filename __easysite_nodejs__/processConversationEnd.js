...
function processConversationEnd(sessionId, userId, finalAnalytics, userFeedback = null) {
  ...
  if (finalAnalytics.topicsDiscussed.includes('quote')) conversationOutcome = 'lead_generated';else
  if (finalAnalytics.topicsDiscussed.includes('contact')) conversationOutcome = 'contact_requested';else
  if (finalAnalytics.totalMessages >= 5) conversationOutcome = 'information_provided';
